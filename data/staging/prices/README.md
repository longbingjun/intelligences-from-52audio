...
通过 CSV 批量导入产品售价，生成 `data/staging/prices/{id}.json`，构建站点时合并进 `views.market`。
...
```powershell
py -3 scripts/import_prices.py data/staging/prices/your_batch.csv
py -3 scripts/pipeline.py all
```
